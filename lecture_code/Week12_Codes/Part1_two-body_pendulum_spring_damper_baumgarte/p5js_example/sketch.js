...
let K = 10.0;
let C = 2.0;
let L0 = 0.05;   // nominal length of the spring
...
// Compute the constraint violations C (position) and C_dot (velocity)
function compute_constraints(q) {
  let [x1, y1, th1, x2, y2, th2] = q;
  let c = Math.cos(th1), s = Math.sin(th1);
  let dx = x2 - x1, dy = y2 - y1;
  
  let C1 = x1 - l1 * s;  // Position constraint for link 1 (x1 - l1*sin(theta1))
  let C2 = y1 + l1 * c;  // Position constraint for link 1 (y1 + l1*cos(theta1))
  let C3 = c * dx + s * dy;  // Constraint for link 2 (x2 - x1, y2 - y1)
  let C4 = th2 - th1;  // Angle constraint (theta2 - theta1)
  
  return math.matrix([C1, C2, C3, C4]);  // Return all constraints as a matrix
}

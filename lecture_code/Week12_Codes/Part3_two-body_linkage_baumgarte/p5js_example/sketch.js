...
function setup() {
  ...
  let x2 = 2 * l1 * Math.cos(theta1) + l2 * Math.cos(theta2);
  let y2 = 2 * l1 * Math.sin(theta1) + l2 * Math.sin(theta2);
...
function compute_constraints(q) {
  ...
  let C1 = x1 - l1 * c1;
  let C2 = y1 - l1 * s1; 
  let C3 = x2 - 2 * l1 * c1 - l2 * c2;
  let C4 = y2 - 2 * l1 * s1 - l2 * s2;
  let C5 = 2 * l1 * s1 + 2 * l2 * s2;
